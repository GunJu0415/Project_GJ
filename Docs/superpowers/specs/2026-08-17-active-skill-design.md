...
| `UGJCardComponent` | `Ability` 카드에서 `EquipSkill(SkillId)`를 부른다 |
...
| `SkillId`가 테이블에 없음 | `EquipSkill`에서 경고 후 장착하지 않음 |
| 슬롯이 다 참 | 경고 후 카드 낭비. 스킬이 하나뿐인 지금은 발생 불가 |
...
- 스킬 쿨타임·슬롯을 보여주는 HUD가 없다
- 스킬 슬롯이 다 찼을 때 무엇을 버릴지 고르는 UI가 없다. 무기 교체 화면이 인덱스 기반이라 그대로 재사용할 수 있다
